...
class RustLib extends BaseEntrypoint<RustLibApi, RustLibApiImpl, RustLibWire> {
  ...
  @override
  int get rustContentHash => 730467754;
...
abstract class RustLibApi extends BaseApi {
  ...
  List<ScreenLocation> crateApiDartTypesGeoLocationPointsToScreen({
    required List<GeoLocation> points,
  ...
  Stream<RenderedMapFrame> crateApiGalileoApiStreamRenderedMapFrames({
    required int sessionId,
  });
}
...
class RustLibApiImpl extends RustLibApiImplPlatform implements RustLibApi {
  ...
  List<ScreenLocation> crateApiDartTypesGeoLocationPointsToScreen({
    ...
    return handler.executeSync(
      SyncTask(
        callFfi: () {
          final serializer = SseSerializer(generalizedFrbRustBinding);
          sse_encode_list_geo_location(points, serializer);
        ...
        codec: SseCodec(
          decodeSuccessData: sse_decode_list_screen_location,
          decodeErrorData: null,
        ),
        constMeta: kCrateApiDartTypesGeoLocationPointsToScreenConstMeta,
        argValues: [points, height, width, vp],
        apiImpl: this,
      ),
    );
  }

  TaskConstMeta get kCrateApiDartTypesGeoLocationPointsToScreenConstMeta =>
      const TaskConstMeta(
        debugName: "geo_location_points_to_screen",
        argNames: ["points", "height", "width", "vp"],
      );
  ...
  Stream<RenderedMapFrame> crateApiGalileoApiStreamRenderedMapFrames({
    ...
    final sink = RustStreamSink<RenderedMapFrame>();
    unawaited(
      handler.executeNormal(
        NormalTask(
          callFfi: (port_) {
            final serializer = SseSerializer(generalizedFrbRustBinding);
            sse_encode_u_32(sessionId, serializer);
            sse_encode_StreamSink_rendered_map_frame_Sse(sink, serializer);
            pdeCallFfi(
              generalizedFrbRustBinding,
              serializer,
              funcId: 24,
          ...
          constMeta: kCrateApiGalileoApiStreamRenderedMapFramesConstMeta,
  ...
  TaskConstMeta get kCrateApiGalileoApiStreamRenderedMapFramesConstMeta =>
      const TaskConstMeta(
        debugName: "stream_rendered_map_frames",
  ...
  @protected
  RustStreamSink<RenderedMapFrame> dco_decode_StreamSink_rendered_map_frame_Sse(
    dynamic raw,
  ) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
  ...
  @protected
  List<ScreenLocation> dco_decode_list_screen_location(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    return (raw as List<dynamic>).map(dco_decode_screen_location).toList();
  }
  ...
  @protected
  RenderedMapFrame dco_decode_rendered_map_frame(dynamic raw) {
    // Codec=Dco (DartCObject based), see doc to use other codecs
    final arr = raw as List<dynamic>;
    if (arr.length != 3)
      throw Exception('unexpected arr length: expect 3 but see ${arr.length}');
    return RenderedMapFrame(
      sequence: dco_decode_u_64(arr[0]),
      viewport: dco_decode_map_viewport(arr[1]),
      mapSize: dco_decode_map_size(arr[2]),
    );
  }
  ...
  @protected
  RustStreamSink<RenderedMapFrame> sse_decode_StreamSink_rendered_map_frame_Sse(
  ...
  @protected
  List<ScreenLocation> sse_decode_list_screen_location(
    SseDeserializer deserializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs

    var len_ = sse_decode_i_32(deserializer);
    var ans_ = <ScreenLocation>[];
    for (var idx_ = 0; idx_ < len_; ++idx_) {
      ans_.add(sse_decode_screen_location(deserializer));
    }
    return ans_;
  }
  ...
  @protected
  RenderedMapFrame sse_decode_rendered_map_frame(SseDeserializer deserializer) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    var var_sequence = sse_decode_u_64(deserializer);
    var var_viewport = sse_decode_map_viewport(deserializer);
    var var_mapSize = sse_decode_map_size(deserializer);
    return RenderedMapFrame(
  ...
  @protected
  void sse_encode_StreamSink_rendered_map_frame_Sse(
    RustStreamSink<RenderedMapFrame> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_String(
      self.setupAndSerialize(
        codec: SseCodec(
          decodeSuccessData: sse_decode_rendered_map_frame,
          decodeErrorData: sse_decode_AnyhowException,
        ),
      ),
      serializer,
  ...
  @protected
  void sse_encode_list_screen_location(
    List<ScreenLocation> self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_i_32(self.length, serializer);
    for (final item in self) {
      sse_encode_screen_location(item, serializer);
    }
  }
  ...
  @protected
  void sse_encode_rendered_map_frame(
    RenderedMapFrame self,
    SseSerializer serializer,
  ) {
    // Codec=Sse (Serialization based), see doc to use other codecs
    sse_encode_u_64(self.sequence, serializer);
    sse_encode_map_viewport(self.viewport, serializer);
    sse_encode_map_size(self.mapSize, serializer);
  }
